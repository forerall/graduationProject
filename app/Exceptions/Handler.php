...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Exception $exception)
    {
        if ($exception instanceof AuthorizationException) {
            if (Input::ajax()) {
                return response()->json(Output::Fail('没有权限'));
            } else {
        ...
        if ($exception instanceof ValidationException) {
            if (Input::ajax()) {
                return response()->json(Output::ParameterError($exception->validator->errors()->toArray()));
            }
        }
        ...
        if (config('app.debug') === false) {
            ...
            //测试环境ajax请求
            if (Input::ajax()) {
                //return response()->json(Output::Fail($exception->getMessage()));
            }
        }
