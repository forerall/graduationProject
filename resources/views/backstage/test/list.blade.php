...
<a href="/backstage/test/create" class="btn btn-sm">新增</a>
</div>
            <div class="col-xs-12">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover list">
                        ...
                        <tr>
                            <th><div>ID</div></th>
<th><div>article_id</div></th>
<th><div>area_id</div></th>
<th><div>创建时间</div></th>
<th><div>操作</div></th>
                        </tr>
                        </thead>
...
<td><div>{{$item->area_id}}</div></td>
<td><div>{{$item->created_at}}</div></td>
<td width="120">
<div class="  action-buttons">
<a class="blue showBtn" href="/backstage/test/{{$item->id}}"><i class="icon-zoom-in bigger-130"></i></a>
<a class="green editBtn" href="/backstage/test/{{$item->id}}/edit"><i class="icon-pencil bigger-130"></i></a>
<a class="red delBtn" href="javascript:void(0)"><i class="icon-trash bigger-130"></i></a>
</div>
</td></tr>
                        @empty
                            <tr>
                                <td colspan="6" class="no-record">没有记录！</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
...
@section('script')
    <script>
        ...
        $(function () {
            $(".delBtn").click(function(){var id = $(this).closest("tr").data("item-id");ajaxConfirm("{{url('/backstage/test')}}/"+id,{},"DELETE",function(data){location.reload();},"确认删除");});
        })
    </script>
@stop